...
def changeSettingDesc(frame, setting):
    ...
    frame.bind('<Enter>', func=lambda e: \
               desc.config(text=descList[setting]
               ))
    frame.bind('<Leave>', func=lambda e: \
               desc.config(text='Setting Description: (appears on mouse hover)'
               ))
...
def validateSettings():
    ...
    if all(isValid.values()):
        status.config(text='Changes saved successfully!',
                      foreground=fgColor)
        printSettings()
        disableSave()
    else:
...
sliderMin = ttk.Scale(
    ...
    to=0,
    orient='vertical',
    command=minSliderChanged,
    variable=sliderMinVal,
    )
sliderMin.set(0.0)
...
sliderMax = ttk.Scale(
    ...
    to=0,
    orient='vertical',
    command=maxSliderChanged,
    variable=sliderMaxVal,
    )
sliderMax.set(100.0)
...
graphBox = left.create_rectangle(
    80, 100, 280, 300,
    fill=lightGrey,
    outline=lightGrey,
    )
...
box1 = round_rectangle(  # webcam device
    right,
    142, 80,
    115, 35,
    radius=15,
...
box2 = round_rectangle(  # sample count
    right,
    400, 80,
    115, 35,
    radius=15,
...
box3 = round_rectangle(  # threshold
    right,
    142, 140,
    115, 35,
    radius=15,
...
box4 = round_rectangle(  # time interval
    right,
    400, 140,
    115, 35,
    radius=15,
...
wbdevLabel = Frame(right, width=125, height=30, bg=grey,
...
ttk.Style().configure('norm.TMenubutton', background=lightGrey,
                      foreground='white', highlightthickness=0)
wbdevValue = ttk.OptionMenu(wbdevInput,
                            selectedWebcam,
                            wbdevList[0],
...
sampleLabel = Frame(right, width=125, height=30, bg=grey,
                    cursor='question_arrow')
...
threshLabel = Frame(right, width=125, height=30, bg=grey,
                    cursor='question_arrow')
...
intvLabel = Frame(right, width=125, height=30, bg=grey,
                  cursor='question_arrow')
...
enableStartupVar.trace('w', lambda name, index, mode, \
                       enableStartupVar=enableStartupVar: \
                       enableSave(enableStartupVar))
...
enableStartup = Frame(right, width=190, height=30, bg=grey,
                      cursor='question_arrow')
...
disablewcpvVar.trace('w', lambda name, index, mode, \
                     disablewcpvVar=disablewcpvVar: \
                     enableSave(disablewcpvVar))
...
disablewcpv = Frame(right, width=190, height=30, bg=grey,
                    cursor='question_arrow')
